...
def train_partial_sums_mnist(
    ...
    if save_metrics_flag:
        today = date.today().isoformat()
        filename = f"partial_sums_full_precision_perf_metrics_cores_{model.required_cores()}_acc_{metrics_history['test_accuracy'][-1]*100:.0f}_{today}.pkl"
...
if __name__ == "__main__":
    num_repeats=30
    print("**"*60)
    print(f"Partial Sums Network on MNIST Performance Tests, {num_repeats} repeats")
    print("**"*60)
    # run the training loop
    trained_model = train_partial_sums_mnist(
        model=model,
        optimizer=optimizer,
        train_ds=train_ds,
        valid_ds=valid_ds,
