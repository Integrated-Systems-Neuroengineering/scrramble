"""
Orchestrator module for scrramblexres tests on CIFAR100.
"""
import subprocess
import itertools
import os
...
## Fixed Parameters
CAPSULE_SIZE = 256
CAPSULE_SIZES = [20, 10]  # Single fixed configuration
BATCH_SIZE = 64
LEARNING_RATE = 3e-4
TRAIN_STEPS = int(5e4)
EVAL_EVERY = int(5e3)
...
## Paths
SCRIPT_PATH = Path(__file__).parent / "scrramblexres_cifar100.py"
RESULTS_CSV = f"/Volumes/export/isn/vikrant/Data/scrramble/logs/scrramble_resnet20_cifar100_sweep_results_{today}.csv"
LOG_DIR = Path(f"/local_disk/vikrant/scrramble/scrramblexres_sweep_logs/logs_scrramble_resnet20_cifar100_sweep_{today}")

LOG_DIR.mkdir(parents=True, exist_ok=True)

## Check if the config was already run
def check_if_already_run(conn_density, slot_size, resample):
    ...
    try:
        ...
        mask = (
            ...
            (df['resample'] == resample)  # ← Fixed: was 'repeat'
        )
        return mask.any()
...
def run_single_config(connection_density, slot_size, resample_idx, config_idx, total_configs):
    """
    Runs a single config of ScRRAMBLE-RES on CIFAR100.
    """
...
def main():
    # Create configs without CAPSULE_SIZES (it's fixed)
    ...
    print(f"Capsule sizes (fixed): {CAPSULE_SIZES}")
    ...
    for i, (p, ls, r) in enumerate(configs, 1):
        was_already_done = check_if_already_run(p, ls, r)
        
        if was_already_done:
            skipped += 1
            continue
            
        successful_run = run_single_config(
